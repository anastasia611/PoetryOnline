...
public class ApiExample {
    ...
    public static void main(String[] args)
        ...
        YouTube.Captions.List request = youtubeService.captions()
                .list(new ArrayList<String>(), "P9LG1kTH3nE");
        CaptionListResponse response = request.execute();
        System.out.println(response);

        OutputStream output = new FileOutputStream("YOUR_FILE");
        YouTube.Captions.Download request2 = youtubeService.captions()
                .download(response.getItems().get(0).getId());
        request2.getMediaHttpDownloader();
        request2.executeMediaAndDownloadTo(output);
    }
}